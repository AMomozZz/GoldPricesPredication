...
def backfill_predictions_for_monitoring(exchange_fg, gold_df, monitor_fg, model):
    l = []
    for i in exchange_fg.features:
        if i.name != 'date':
            l.append(i.name)
    features_df = exchange_fg.read()
    features_df = features_df.sort_values(by=['date'], ascending=True)
    features_df = features_df.tail(10)
    features_df['predicted_close_price'] = model.predict(features_df[l])
    df = pd.merge(features_df, gold_df[['date', 'close_price']], on="date")
    df['days_before_forecast_day'] = 1
    hindcast_df = df
    df = df.drop('close_price', axis=1)
